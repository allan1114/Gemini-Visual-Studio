...
/** Persists records to localStorage with their `key` field encrypted at rest. */
async function persistKeys(records: ApiKeyRecord[]): Promise<void> {
  const encrypted = await Promise.all(
    records.map(async (r) => ({ ...r, key: await encryptString(r.key) }))
  );
  localStorage.setItem(STORAGE_KEYS.API_KEYS, JSON.stringify(encrypted));
}

/** Loads records from localStorage, decrypting the `key` field for display/use. */
async function loadKeys(): Promise<ApiKeyRecord[]> {
  const saved = localStorage.getItem(STORAGE_KEYS.API_KEYS);
  if (!saved) return [];
  const parsed: ApiKeyRecord[] = JSON.parse(saved);
  return Promise.all(parsed.map(async (r) => ({ ...r, key: await decryptString(r.key) })));
}
...
const KeyWallet: React.FC<KeyWalletProps> = ({ t }) => {
  ...
  useEffect(() => {
    loadKeys().then((loaded) => {
      if (loaded.length > 0) {
        setKeys(loaded);
        // Re-persist so any legacy plaintext keys get encrypted at rest.
        persistKeys(loaded).catch(() => {});
      }
    });
  ...
  const saveKeys = (updated: ApiKeyRecord[]) => {
    setKeys(updated);
    // Encrypt the key material before it touches localStorage.
    persistKeys(updated).catch((err) => console.error('Failed to persist keys:', err));
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in">
        ...
        <div className="p-6 bg-amber-500/10 border border-amber-500/20 rounded-[2rem] flex items-start gap-4 animate-in slide-in-from-top duration-500">
          ...
          <div>
            <h4 className="text-amber-500 font-black uppercase tracking-widest text-xs mb-1">
              AI Studio Key Required
            </h4>
            <p className="text-amber-500/70 text-[11px] leading-relaxed font-medium">
              To use high-end models like <b>Gemini 3 Pro</b> or <b>Veo</b>, you must select an API
              key from your Google Cloud project. Click the "Select Studio Key" button above to open
              the platform dialog.
              <a
      ...
      <div className="grid grid-cols-1 gap-4">
        {keys.map((k) => (
          <div
            key={k.id}
            className={`glass p-6 rounded-[2rem] border-2 transition-all flex items-center justify-between ${k.isActive ? 'border-indigo-500 bg-indigo-500/5' : 'border-white/5 hover:border-white/10'}`}
          >
            <div className="flex items-center gap-6">
              ...
              >
                {k.isActive ? (
                  ...
                  <i className="fa-solid fa-circle text-xs opacity-20"></i>
                )}
              </div>
