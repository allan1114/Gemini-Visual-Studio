...
const EDIT_DEFAULT_PROMPT =
  '根據上載既照片來拍攝各種不同凡響既人像攝影。女神角式樣貌身材都必需一樣，而每張獨立照片都不同動作/表情，造型服裝要一致。';
const DEFAULT_NEGATIVE =
  '避免低飽和、多餘四肢、惡劣照明、模糊噪音、卡通插圖、動漫3D、扭曲解蓬、過度磨皮、塑膠感皮膚、過度銳化、人物擠、錯數量比例、服飾皮膚粗糙、色調失和不足、形體失真、姿勢僵硬、配件突兀、場景雜亂、臉部扭曲、五官錯位、比例異常、多手指、多肢體、避免Al感。';
...
const EditView: React.FC<EditViewProps> = ({
  ...
  const handleManifest = async () => {
    ...
    const visualDirectives = combinedTags
      ? `Visual directives to strictly follow: ${combinedTags}.`
      : '';
    ...
    const finalPrompt = isHealing
      ? prompt
      : [visualDirectives, `User Intent: ${prompt}`, `Aesthetic Style: ${fashionContext}`]
          .filter(Boolean)
          .join('\n');
    ...
    if (isLocalMode && currentMask) {
      ...
      if (isSmartRefine) {
        ...
        try {
          ...
          semanticTarget = await GeminiService.analyzeMaskedRegion(
            sourceImg.url,
            currentMask,
            prompt
          );
      ...
      await generateInpaint(
        finalPrompt,
        { ...sourceImg, mask: currentMask },
        { model, aspectRatio, imageSize, negativePrompt, seed, temperature, semanticTarget }
      );
    } else {
      await generateSingle(
        finalPrompt,
        { model, aspectRatio, imageSize, negativePrompt, seed, temperature },
        { ...sourceImg, context: prompt, type: 'edit' }
      );
    }
  };
  ...
  const handleApplyAdvanced = (newUrl: string) => {
    ...
    GeminiService.generateMetadata(newUrl, 'image/webp')
      .then((metadata) => {
        setPreviews((prev) =>
          prev.map((p) =>
            p.id === newPreview.id
        ...
        <div className="glass p-8 rounded-[2.5rem] space-y-8 border-indigo-500/10">
            ...
            {!sourceImg ? (
              <div className="w-full h-full glass border-dashed border-2 border-indigo-500/20 rounded-3xl flex items-center justify-center relative overflow-hidden bg-black/20 group">
                <div className="text-center py-4">
                  ...
                  />
                  <div className="w-16 h-16 bg-indigo-500/10 rounded-full flex items-center justify-center text-indigo-400 mx-auto mb-4 group-hover:scale-110 transition-transform">
              ...
              <div className="w-full h-full relative group shadow-2xl rounded-3xl overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-6 right-6 bg-black/80 p-3 rounded-full hover:bg-red-500 transition-all text-xs z-30"
                >
          ...
          <div className="space-y-4">
            ...
            >
              ...
              {chips.map((b) => (
                <div
                  key={b.id}
                  className={`px-4 py-2 bg-gradient-to-br ${b.color || 'from-indigo-600 to-purple-600'} rounded-xl flex items-center gap-2 shadow-lg group animate-in zoom-in duration-200`}
                >
          ...
          >
            {isLoading ? (
              ...
              <i className="fa-solid fa-wand-sparkles mr-3 text-lg"></i>
            )}
            {isSmartAnalyzing
              ? 'Analyzing Mask...'
              : isRemovingBg
          ...
          <div className="glass p-8 rounded-[3rem] space-y-8 shadow-2xl bg-gradient-to-b from-transparent to-indigo-500/5 border border-indigo-500/20">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button
                ...
                className="px-8 py-5 bg-red-500/10 rounded-2xl text-red-400 hover:bg-red-500/20 transition-all border border-red-500/10"
              >
