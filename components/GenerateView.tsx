...
const GenerateView: React.FC<GenerateViewProps> = ({
  ...
  const handleManifest = async () => {
    ...
    await generateSingle(fullPrompt, {
      model,
      aspectRatio,
      imageSize,
      negativePrompt,
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in">
      <div className="lg:col-span-8 space-y-8">
        <div className="glass p-8 rounded-[2.5rem] border-white/10 space-y-8 relative overflow-hidden">
          ...
          <div className="flex justify-between items-center relative z-10">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="text-indigo-400 hover:text-indigo-300 text-[10px] font-bold uppercase transition-colors flex items-center gap-1"
              >
                <i className="fa-solid fa-file-arrow-up"></i> {t.uploadPrompt}
              </button>
              <button
                onClick={() => {
                  setPrompt('');
          ...
          <div className="space-y-6 relative z-10">
            ...
            >
              ...
              {chips.length === 0 && (
                <p className="text-gray-600 text-xs font-black uppercase tracking-[0.2em] pointer-events-none">
                  {t.dropHint}
                </p>
              )}
              {chips.map((b) => (
                <div
                  key={b.id}
                  className={`px-4 py-2.5 bg-gradient-to-br ${b.color || 'from-indigo-600 to-purple-600'} rounded-2xl flex items-center gap-2 shadow-xl shadow-black/20 group animate-in zoom-in duration-200`}
                >
            ...
            <div
              className={`transition-all duration-500 p-[1px] rounded-[2rem] bg-gradient-to-b ${isPromptFocused ? 'from-indigo-500 via-purple-500 to-indigo-500 shadow-[0_0_50px_rgba(99,102,241,0.15)]' : 'from-white/10 to-transparent'}`}
            >
              <div className="bg-[#0c0c0c] rounded-[2rem] p-6 space-y-4 relative overflow-hidden group/prompt">
                ...
                <div className="relative z-10">
                  ...
                  >
                    {isExpanding ? (
                      <i className="fa-solid fa-wand-sparkles fa-spin text-lg"></i>
                    ) : (
                ...
                <div className="flex items-center justify-between gap-4 pt-2 relative z-10">
                  ...
                  >
                    {isSuggesting ? (
                      ...
                      <i className="fa-solid fa-lightbulb group-hover/inspire:scale-110 transition-transform"></i>
                    )}{' '}
                    {t.inspire}
                  </button>
          ...
          <button
            ...
            disabled={
              ...
              (model === 'pro' && (window as any).aistudio && !hasStudioKey)
            }
          ...
          >
            {isLoading ? (
              ...
              <i className="fa-solid fa-sparkles mr-3 text-lg group-hover/gen:rotate-12 transition-transform"></i>
            )}
        ...
        {isLoading && previews.length === 0 && (
          <div
            className={`glass p-4 rounded-[2.5rem] border-white/10 shadow-2xl bg-white/[0.02] animate-pulse`}
          >
            <div
              className={`w-full ${getRatioClass(aspectRatio)} bg-white/5 rounded-[2rem] flex flex-col items-center justify-center gap-4 relative overflow-hidden`}
            >
              ...
              <div className="flex flex-col items-center">
                <p className="text-indigo-400/40 text-[10px] font-black uppercase tracking-[0.3em]">
                  {t.synthesizing}
                </p>
        ...
        {previews.length > 0 && (
          <div className="glass p-8 rounded-[3rem] space-y-8 animate-in slide-in-from-bottom duration-500 shadow-2xl bg-gradient-to-b from-transparent to-indigo-500/5 border border-indigo-500/10">
            ...
            <div className="relative group/img-preview">
              <img
                src={previews[selectedPreviewIdx]?.url}
                className="w-full rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] border border-white/10 transition-transform duration-700"
                alt="Result"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                disabled={
                  storingIds.has(previews[selectedPreviewIdx].id) ||
                  savedIds.has(previews[selectedPreviewIdx].id)
                }
              ...
              {previews.length > 1 && (
                <button
                  ...
                  className="flex-1 py-5 bg-purple-600 hover:bg-purple-500 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95"
                >
              ...
              <button
                ...
                className="flex-1 py-5 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 rounded-2xl font-black text-xs uppercase tracking-widest text-indigo-400 transition-all hover:shadow-indigo-500/10 active:scale-95"
              >
              ...
              <button
                onClick={() => setPreviews([])}
                className="px-8 py-5 bg-red-500/10 hover:bg-red-500/20 rounded-2xl text-red-400 border border-red-500/20 transition-all active:scale-95"
              >
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="glass p-8 rounded-[2rem] border-white/5 relative overflow-hidden">
          ...
          <h3 className="font-black mb-6 text-[10px] uppercase tracking-[0.2em] text-indigo-400 border-b border-white/5 pb-4 relative z-10">
            {t.tuning}
          </h3>
          <div className="relative z-10">
            <TuningControls
