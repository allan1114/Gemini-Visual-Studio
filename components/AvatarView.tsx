...
const DEFAULT_NEGATIVE =
  '避免低飽和、多餘四肢、惡劣照明、模糊噪音、卡通插圖、動漫3D、扭曲解蓬、過度磨皮、塑膠感皮膚、過度銳化、人物擠、錯數量比例、服飾皮膚粗糙、色調失和不足、形體失真、姿勢僵硬、配件突兀、場景雜亂、臉部扭曲、五官錯位、比例異常、多手指、多肢體、避免Al感、不要加入text除非prompt有要求。';
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in">
        ...
        <div className="glass p-8 rounded-[2.5rem] space-y-8 border-indigo-500/10 bg-gradient-to-tr from-transparent via-indigo-500/[0.01] to-transparent shadow-2xl">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-black flex items-center gap-3">
              <i className="fa-solid fa-user-astronaut text-indigo-400"></i>
              {t.avatar}
            </h3>
            <button
              onClick={() => {
              ...
              className="text-gray-500 hover:text-red-400 text-[10px] font-bold uppercase"
            >
          ...
          <div className="flex gap-4 p-1 bg-black/40 rounded-2xl border border-white/5">
            <button
              ...
              className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase transition-all ${mode === 'transform' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
            >
              {t.transformPhoto}
            </button>
            <button
              onClick={() => setMode('scratch')}
              className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase transition-all ${mode === 'scratch' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
            >
              {t.createFromScratch}
            </button>
          </div>
            ...
            <div className="glass p-8 border-dashed border-2 border-indigo-500/20 rounded-3xl flex items-center justify-center relative overflow-hidden bg-black/20 group">
              {!sourceImg ? (
                <div className="text-center py-4">
                  ...
                  />
                  <div className="w-12 h-12 bg-indigo-500/10 rounded-full flex items-center justify-center text-indigo-400 mx-auto mb-3 group-hover:scale-110 transition-transform">
                    <i className="fa-solid fa-cloud-arrow-up text-xl"></i>
                  </div>
                  <p className="font-bold uppercase tracking-widest text-[10px] text-gray-500">
                    {t.uploadSource}
                  </p>
                </div>
              ) : (
                <div className="relative w-full flex flex-col items-center">
                  <img
                    src={sourceImg.url}
                    className="max-h-48 rounded-xl shadow-xl border border-white/10"
                    alt="Source"
                  />
                  <button
                    onClick={() => setSourceImg(null)}
                    className="absolute top-2 right-2 bg-black/60 p-2 rounded-full hover:bg-red-500 transition-all text-xs z-20"
                  >
          ...
          <div className="grid grid-cols-4 gap-2">
            {AVATAR_STYLES.map((s) => (
              <button
                ...
                className={`py-3 rounded-xl text-[10px] font-bold uppercase transition-all ${styleId === s.id ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/50 shadow-lg shadow-indigo-500/10' : 'bg-white/5 text-gray-500 border border-white/5 hover:border-white/10'}`}
              >
                {s.label[language]}
              </button>
            ))}
          ...
          >
            {chips.length === 0 && (
              <p className="text-gray-600 text-[10px] font-black uppercase tracking-widest pointer-events-none text-center px-4">
                {t.dropHint}
              </p>
            )}
            {chips.map((b) => (
              <div
                key={b.id}
                className={`px-4 py-2 bg-gradient-to-br ${b.color || 'from-indigo-600 to-purple-600'} rounded-xl flex items-center gap-2 shadow-lg group`}
              >
                <i className={`fa-solid ${b.icon || 'fa-tag'} text-[9px]`}></i>
                <span className="text-[10px] font-black uppercase tracking-tight">{b.label}</span>
                <button
                  onClick={() => setChips((prev) => prev.filter((c) => c.id !== b.id))}
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              className="w-full bg-black/40 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-medium resize-y min-h-[100px]"
              placeholder={t.subjectPlaceholder}
            />
          </div>

          <div className="space-y-4">
            <label className="text-[10px] font-black text-red-500/60 uppercase tracking-widest">
              {t.negativeLabel}
            </label>
            <textarea
              value={negativePrompt}
              onChange={(e) => setNegativePrompt(e.target.value)}
              className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 h-24 text-[11px] outline-none resize-y"
            />
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-6 bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-600 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all flex items-center justify-center group/gen"
          >
            {isLoading ? (
              <i className="fa-solid fa-circle-notch fa-spin mr-3 text-lg"></i>
            ) : (
              <i className="fa-solid fa-sparkles mr-3 text-lg"></i>
            )}
        ...
        {previews.length > 0 && (
          <div className="glass p-8 rounded-[3rem] space-y-8 shadow-2xl bg-gradient-to-b from-transparent to-indigo-500/5 border border-indigo-500/10 animate-in slide-in-from-bottom duration-500">
            <div className="relative group/img-preview">
              <img
                src={previews[0]?.url}
                className="w-full rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] border border-white/10"
                alt="Result"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                disabled={isStoring}
                className={`flex-1 py-5 rounded-2xl font-black text-xs tracking-widest uppercase shadow-xl transition-all ${isSaved ? 'bg-green-600' : 'bg-indigo-600 hover:bg-indigo-500'}`}
              >
                {isStoring ? (
                ...
                {isStoring ? t.storing : isSaved ? t.saved : t.store}
              </button>
              <button
                onClick={() => setPreviews([])}
                className="px-8 py-5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl transition-all hover:bg-red-500/20"
              >
                <i className="fa-solid fa-trash"></i>
              </button>
            </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="glass p-8 rounded-3xl border-white/5 shadow-xl">
          <h3 className="font-black mb-6 text-[10px] uppercase tracking-widest text-indigo-400 border-b border-white/10 pb-4">
            {t.tuning}
          </h3>
          <TuningControls
            currentModel={model}
