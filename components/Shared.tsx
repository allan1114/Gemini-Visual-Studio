  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 gap-2 mb-6">
      <button
        ...
        className={`p-4 rounded-xl border-2 text-left transition-all ${currentModel === 'flash' ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/5'}`}
      >
        <div className="text-xs font-black uppercase">{t.engineFlash}</div>
      </button>
      <button
        onClick={() => onModelChange('pro')}
        className={`p-4 rounded-xl border-2 text-left transition-all ${currentModel === 'pro' ? 'border-purple-500 bg-purple-500/10' : 'border-white/5 bg-white/5'}`}
      >
        <div className="text-xs font-black uppercase">{t.enginePro}</div>
      </button>
      {showImagen &&
        (['imagen-4', 'imagen-4-fast', 'imagen-4-ultra'] as const).map((m) => (
          <button
            key={m}
            onClick={() => onModelChange(m)}
            className={`p-4 rounded-xl border-2 text-left transition-all ${currentModel === m ? 'border-emerald-500 bg-emerald-500/10' : 'border-white/5 bg-white/5'}`}
          >
